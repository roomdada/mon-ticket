...
<div class="leading-tight">
  ...
  <x-lv-drop-down :dropDownWidth="64">
    <x-slot name="trigger">
      <x-lv-select-button>
        {{ __('Trir par') }}@if ($sortableByName = $sortableBy->flip()->get($sortBy)):
    ...
    <x-lv-drop-down.header label="{{ __('Trier par') }}" />

    {{-- Each sortable item --}}
    @foreach ($sortableBy as $title => $column)
      <a href="#!"
        wire:click.prevent="sort('{{ $column }}')"
        title="{{ __('Trier par') }} {{ $title }} {{ __($sortOrder == 'asc' ? 'descending' : 'ascending') }}"
